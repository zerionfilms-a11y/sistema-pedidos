...
function inserirCabecalhoContinuacao(pageEl){
  // Mantido por compatibilidade, mas sem banner de continuação.
  // Isso evita ocupar espaço extra nas páginas seguintes.
  return pageEl
}
...
function construirPaginaPdfBase(root, rows, opts={}){
  ...
  page.className = root.className || 'pdf-root'
  page.setAttribute('style', root.getAttribute('style') || '')
  ...
  if (primeiro) {
    if (children[0]) page.appendChild(children[0].cloneNode(true))
    if (children[1]) page.appendChild(children[1].cloneNode(true))
  }

  const tableSrc = root.querySelector('.pdf-table')
  if (tableSrc) {
    const tableWrapper = tableSrc.cloneNode(true)
  ...
  if (ultimo) {
    ...
    if (obs) page.appendChild(obs.cloneNode(true))
...
function medirAlturaPaginaPdf(root, rows, opts={}){
  const host = criarHostPdfTemporario()
  const page = construirPaginaPdfBase(root, rows, opts)
...
function montarPaginasPdfEstruturadas(root){
  ...
  const paginas = []
  const basePrimeira = medirAlturaPaginaPdf(root, [], { primeiro: true, ultimo: false })
  const baseOutras = medirAlturaPaginaPdf(root, [], { primeiro: false, ultimo: false })
  const safety = 12

  const alturasLinhas = rows.map(row => {
    const host = criarHostPdfTemporario()
    const probe = construirPaginaPdfBase(root, [row], { primeiro: false, ultimo: false })
    host.appendChild(probe)
    const altura = Math.max(0, probe.scrollHeight - baseOutras)
    host.remove()
    return altura || row.getBoundingClientRect().height || 40
  })

  let idx = 0
  while (idx < rows.length) {
    const primeiro = paginas.length === 0
    const base = primeiro ? basePrimeira : baseOutras
    const limite = Math.max(200, PDF_PAGE_HEIGHT - base - safety)
    let usado = 0
    const currentRows = []

    while (idx < rows.length) {
      const alturaLinha = alturasLinhas[idx]

      if (currentRows.length > 0 && (usado + alturaLinha) > limite) break

      if (currentRows.length === 0 && alturaLinha > limite) {
      ...
      if ((usado + alturaLinha) > limite) break
...
function prepararItensTempPedido(itens=[]){
  const produtos = storageGet(KEYS.PROD)
  return (Array.isArray(itens) ? itens : []).map(it => {
    const produtoId = it.produtoId || it.produto?.id || ''
    const produtoAtual = produtos.find(p => p.id === produtoId)
    const produto = produtoAtual || it.produto || {
      id: produtoId || uid(),
      nome: it.nome || 'Produto não encontrado',
...
function abrirModalPedido(pedido=null, modo='novo'){
  const editando = !!(pedido?.id && modo !== 'duplicar')
  const duplicando = !!(pedido && modo === 'duplicar')
  pedidoEditandoId = editando ? pedido.id : null
  itensTempPed = pedido?.itens ? prepararItensTempPedido(pedido.itens) : []
  document.getElementById('modalPedido').style.display='flex'
  document.getElementById('pedidoModalTitle').textContent = editando ? 'Editar Pedido' : (duplicando ? 'Duplicar Pedido' : 'Novo Pedido')
  document.getElementById('btnSalvarPedido').textContent = editando ? 'Salvar Alterações' : (duplicando ? 'Salvar Novo Pedido' : 'Salvar Pedido')
  ...
  document.getElementById('pedStatusInicial').value = duplicando ? 'produzindo' : (pedido?.status || 'produzindo')
  ...
  mostrarNotaAtual(editando ? pedido : null)
}
...
function renderPedidos(){
  ...
  pedidos.slice().reverse().forEach(p=>{
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end">
      ...
      <button onclick='duplicarPedido("${p.id}")'>Duplicar</button>
...
function duplicarPedido(id){
  ...
  // Abre um novo pedido já preenchido com os dados do pedido original.
  // Não copia número, data, status final, rastreio nem nota, porque isso deve ser novo.
  abrirModalPedido({
    ...p,
    id: null,
    numero: '',
...
function montarHtmlPedidoPdf(p){
  const contatoHtml = [
    p.canal ? `<strong>Canal:</strong> ${escapeHtml(canalLabel(p.canal))}` : '',
    ...
    p.docTipo || p.documento || p.cpf ? `<div style="margin-top:4px"><strong>${escapeHtml(pegarDocumentoPedido(p).label)}:</strong> ${escapeHtml(pegarDocumentoPedido(p).valor || '-')}</div>` : '',
    p.endereco ? `<div style="margin-top:4px"><strong>Endereço:</strong> ${escapeHtml(p.endereco)}</div>` : '',
    p.freteTipo || p.freteValor || p.freteObs ? `<div style="margin-top:4px"><strong>Frete:</strong> ${escapeHtml(p.freteTipo || 'Outro')} • ${escapeHtml(formatMoney(p.freteValor || 0))}${p.freteObs ? ' • ' + escapeHtml(p.freteObs) : ''}</div>` : '',
    p.status ? `<div style="margin-top:4px"><strong>Status:</strong> ${escapeHtml(statusLabel(p.status))}</div>` : '',
    p.rastreio ? `<div style="margin-top:4px"><strong>Rastreio:</strong> ${escapeHtml(p.rastreio)}</div>` : '',
    p.rastreioObs ? `<div style="margin-top:4px"><strong>Obs. rastreio:</strong> ${escapeHtml(p.rastreioObs)}</div>` : ''
  ].filter(Boolean).join('')
  const itens = p.itens || []
  const subtotal = Number(p.subtotal ?? itens.reduce((s,it)=>s + Number(it.preco || 0) * Number(it.qtd || 0), 0))
  const total = Number(p.total ?? subtotal)
  return montarHtmlDocumento('Pedido', p.numero, p.cliente, contatoHtml, itens, subtotal, p.desconto || 0, total, p.obs || '')
}
