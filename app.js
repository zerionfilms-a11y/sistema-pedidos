...
function freteLabel(f){
  const map = { PAC:'PAC', SEDEX:'SEDEX', Outro:'Outro' }
  return map[f] || 'Outro'
}
...
function montarHtmlDocumento(tipo, numero, cliente, detalhesTopo, itens, subtotal, desconto, freteTipo, freteValor, total, obs=''){
  ...
  <div style="width:794px;min-height:1123px;padding:32px 36px;box-sizing:border-box;background:#fff;color:#000;font-family:Arial,Helvetica,sans-serif">
    ...
    <div style="display:flex;justify-content:flex-end">
      <div style="min-width:260px">
        ...
        ${Number(freteValor || 0) > 0 ? `<div style="display:flex;justify-content:space-between;padding:4px 0"><span>Frete${freteTipo ? ` (${escapeHtml(freteTipo)})` : ''}</span><strong>R$ ${Number(freteValor).toFixed(2)}</strong></div>` : ''}
...
function calcularTotalItensPed(){ return itensTempPed.reduce((s,it)=>s + it.produto.preco * it.qtd, 0) }
function salvarPedido(){
  ...
  const freteTipo=document.getElementById('pedFreteTipo').value || 'PAC'
  ...
  pedidos.slice().reverse().forEach(p=>{
    ...
    div.innerHTML = `<div>
      ...
      <div class="order-meta">
        ...
        <span>${p.freteTipo ? escapeHtml(freteLabel(p.freteTipo)) : 'Sem frete'}</span>
        <span>${escapeHtml(formatMoney(Number(p.freteValor)||0))}</span>
...
function buildMensagemPedido(p, etapa, rastreio=''){
  ...
  if(etapa === 'produzindo'){
    return `Oi, ${primeiroNome}! Passando para te avisar que o seu pedido já foi produzido.\nAgora ele está aguardando o envio e eu vou te manter atualizado por aqui.`
  }
  if(etapa === 'enviado'){
    const base = `Oi, ${primeiroNome}! Seu pedido já foi enviado 🚚✨`
    const cod = rastreio ? `\nCódigo de rastreio: ${rastreio}` : ''
    const extra = p.rastreioObs ? `\n${p.rastreioObs}` : ''
    return base + cod + extra + `\nAssim que tiver qualquer atualização, eu te aviso por aqui.`
  }
  if(etapa === 'finalizado'){
    return `Oi, ${primeiroNome}! Seu pedido foi finalizado. Muito obrigado pela confiança 🙌`
  }
...
function getContatoParaWhatsApp(p){
  ...
  if(!tel) return ''
  if(tel.length === 11 || tel.length === 10 || tel.length > 11) return tel
  return tel
}
...
function visualizarPedido(id){
  ...
  const topo = `
    ...
    ${p.freteTipo || p.freteValor ? `<div style="margin-top:4px"><strong>Frete:</strong> ${escapeHtml(freteLabel(p.freteTipo))}${p.freteValor ? ` • ${formatMoney(p.freteValor)}` : ''}</div>` : ''}
