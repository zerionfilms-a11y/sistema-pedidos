...
function getEmpresaFooterHtml(){
  return `
    <div style="margin-top:26px;border-top:1px solid #d1d5db;padding-top:12px;font-size:11px;color:#374151;line-height:1.45">
...
function montarHtmlPedidoPdf(p){
  ...
  return `
    <div style="width:794px;min-height:1123px;padding:28px 34px;box-sizing:border-box;background:#fff;color:#111827;font-family:Arial,Helvetica,sans-serif">
      ...
      <div style="display:flex;justify-content:flex-end;margin-top:16px">
        <div style="min-width:300px;border:1px solid #e5e7eb;border-radius:16px;padding:14px;background:#fafafa">
          <div style="display:flex;justify-content:space-between;padding:4px 0;color:#334155"><span>Subtotal produtos</span><strong>${formatMoney(subtotal)}</strong></div>
          <div style="display:flex;justify-content:space-between;padding:4px 0;color:#334155"><span>Frete</span><strong>${formatMoney(freteValor)}</strong></div>
          <div style="display:flex;justify-content:space-between;padding:10px 0;border-top:1px solid #cbd5e1;margin-top:6px;font-size:16px"><span>Total final</span><strong>${formatMoney(total)}</strong></div>
        </div>
      </div>

      ${p.obs ? `<div style="margin-top:16px;border:1px dashed #cbd5e1;border-radius:16px;padding:12px;background:#fff"><strong>Observações:</strong><div style="margin-top:4px;white-space:pre-wrap;line-height:1.5">${escapeHtml(p.obs)}</div></div>` : ''}
...
function montarHtmlDocumento(titulo, numero, cliente, extraInfoHtml, itens, subtotal, desconto, total, observacoes=''){
  const itensHtml = (itens || []).map(it => {
    ...
    const imgHtml = imagem ? `
      <div style="width:150px;height:150px;border-radius:12px;overflow:hidden;background:#f3f4f6;border:1px solid #e5e7eb;flex:0 0 auto">
  ...
  return `
    <div style="width:794px;min-height:1123px;padding:28px 34px;box-sizing:border-box;background:#fff;color:#111827;font-family:Arial,Helvetica,sans-serif">
      ...
      ${observacoes ? `<div style="margin-top:16px;border:1px dashed #cbd5e1;border-radius:16px;padding:12px;background:#fff"><strong>Observações:</strong><div style="margin-top:4px;white-space:pre-wrap;line-height:1.5">${escapeHtml(observacoes)}</div></div>` : ''}
...
function montarHtmlOrcamentoPdf(o){
  ...
  const itens = (o.itens || []).map(it => ({
    ...it,
    imagem: it.imagem || getImagemProdutoDoItem(it)
  }))
