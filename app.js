...
const PDF_PAGE_RESERVED_BOTTOM = 120
...
function construirPaginaPdfBase(root, rows, mostrarExtras, paraMedir = false){
  const page = root.cloneNode(true)
  page.style.width = PDF_PAGE_WIDTH + 'px'
  page.style.height = 'auto'
  page.style.minHeight = paraMedir ? '0px' : PDF_PAGE_HEIGHT + 'px'
...
function montarPaginasPdfEstruturadas(root){
  ...
  while (idx < rows.length) {
    ...
    while (idx < rows.length) {
      ...
      const measuringPage = construirPaginaPdfBase(root, currentRows, false, true)
      host.innerHTML = ''
      host.appendChild(measuringPage)
      const altura = measuringPage.scrollHeight
    ...
    const finalPage = construirPaginaPdfBase(root, currentRows, isLast, false)
...
function montarHtmlDocumento(titulo, numero, cliente, extraInfoHtml, itens, subtotal, desconto, total, observacoes=''){
  const itensHtml = (itens || []).map(it => {
    ...
    const imgHtml = imagem ? `
      <div style="width:120px;height:120px;border-radius:12px;overflow:hidden;background:#f3f4f6;border:1px solid #e5e7eb;flex:0 0 auto">
